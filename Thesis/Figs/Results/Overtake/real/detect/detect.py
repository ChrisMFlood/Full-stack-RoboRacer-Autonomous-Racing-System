...
root_path = "C:/Users/bbdnet2985/Desktop/Matsters/Report/Stellenbosch_University_Electrical_and_Electronic_Engineering_Template__1_/Results/Overtake/real/detect"
...
print(ego_positions[:,1:3].shape)
ego_positions[:,1:3]  += np.random.normal(0, 0.02, ego_positions[:,1:3].shape)  # add small noise to ego positions to simulate GPS noise
opp_positions = np.loadtxt(opp_positions_path, delimiter=',')
print(opp_positions.shape)
opp_positions[:,1:3]  += np.random.normal(0, 0.02, opp_positions[:,1:3].shape)  # add small noise to opp positions to simulate GPS noise
...
for i in range(1,len(detected_positions)):
    if detected_positions[i, 0] - detected_positions[i-1, 0] <= 1/30:
        detected_positions[i, 0] = detected_positions[i-1, 0]

# Apply interpolation to detected positions
interpolated_x = interp_func_x(detected_positions[:, 0])
interpolated_y = interp_func_y(detected_positions[:, 0])

# Combine interpolated values into a single array
interpolated_actual_positions = np.column_stack((detected_positions[:, 0], interpolated_x, interpolated_y))

unique_t, counts = np.unique(detected_positions[:, 0], return_counts=True)
print(f'Unique timestamps in detected positions: {len(unique_t)} out of {detected_positions.shape[0]} total detections')
num = int((detected_positions[-1,0]+(1/15) - detected_positions[0,0])/(1/15)+1)
print(f'num: {num}')
times = np.linspace(detected_positions[0,0], detected_positions[-1,0]+(1/15), num=num)
print(f'Time range for interpolation: {times[0]} to {times[-1]}, total {len(times)} points')

...
valid_times = estimated[:,0]
interpolated_actual_positions = np.zeros_like(estimated)
for i,t in enumerate(valid_times):
    #find closest time in opp_positions
    idx = np.argmin(np.abs(opp_positions[:,0]-t))
    interpolated_actual_positions[i,:] = opp_positions[idx,:]
...
mean_ds = np.mean(ds)
print(f'Mean Distance between vehicles along raceline: {mean_ds:.4f} m')
std_ds = np.std(ds)
print(f'Std Distance between vehicles along raceline: {std_ds:.4f} m')
rmse_ds = np.sqrt(np.mean((1.5-ds)**2))
print(f'RMSE Distance between vehicles along raceline: {rmse_ds:.4f} m')
width = 8/2.54
height = 8/2.54
plt.figure(figsize=(width, height))
